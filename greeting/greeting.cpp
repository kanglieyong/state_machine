...
// only be defined at a point where the state machine is defined.
struct Greeting;
...
// enter when the machine is initialized. That's why Greeting is
...
// belongs to and where it is located in the statechart. Bost is
// specified with Context argument that is passed bo
...
struct Greeting : sc::simple_state<Greeting, Machine>
{
  ...
  // exists the state. Therefore, a state entry action can be
